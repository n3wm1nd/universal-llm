...
toProviderRequestSonnet45 msg = fmap snd $ toProviderRequest anthropicSonnet45NoReason (Model ClaudeSonnet45NoReason Anthropic) msg def
...
toProviderRequestGLM45 msg = fmap snd $ toProviderRequest openAITestPlaceholder (Model TestPlaceholderModel OpenAIProvider.OpenAI) msg def
toProviderRequestGLM45WithReasoning msg = fmap snd $ toProviderRequest openAITestPlaceholder (Model TestPlaceholderModel OpenAIProvider.OpenAI) msg def

-- fromProviderResponse signature: composableProvider -> model -> configs -> state -> response -> (state, messages)
fromProviderResponseGLM45 configs resp = either (error . show) snd $ fromProviderResponse openAITestPlaceholder (Model TestPlaceholderModel OpenAIProvider.OpenAI) configs def resp
