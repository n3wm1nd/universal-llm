...
-- Type alias for the test model (non-reasoning variant)
type TestModel = Model TestModels.ClaudeSonnet45NoReason Provider.AnthropicOAuth

-- Helper to build request for ClaudeSonnet45NoReason
...
-- Helper to build request for ClaudeSonnet45 (with reasoning)
...
-- Helper to parse response for ClaudeSonnet45 (with reasoning)
...
spec getResponse = do
  describe "Anthropic Request Building (no API calls)" $ do
    it "can build and evaluate a request without looping" $ do
      let model = Model ClaudeSonnet45NoReason Provider.AnthropicOAuth
  ...
  describe "Anthropic Composable Provider - Tool Calling" $ do
    ...
    it "completes full tool calling conversation flow" $ do
      let model = Model ClaudeSonnet45NoReason Provider.AnthropicOAuth :: TestModel
  ...
  describe "Anthropic Composable Provider - Streaming with Tools" $ do
    ...
    it "builds correct request for streaming tool calls with reasoning" $ do
      let model = Model ClaudeSonnet45 Provider.AnthropicOAuth :: TestModelWithReasoning
          toolDef = ToolDefinition
            { toolDefName = "search"
  ...
  describe "Compile-Time Safety Demonstrations" $ do
    ...
    it "allows tool use with tool-capable model" $ do
      let model = Model ClaudeSonnet45NoReason Provider.AnthropicOAuth :: TestModel  -- HasTools
